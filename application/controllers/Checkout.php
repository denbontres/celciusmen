...
class Checkout extends CI_Controller {
    ...
    public function index(){
		...
		$order= $this->db->query("SELECT * FROM tbl_pemesanan WHERE nofaktur = '$nofaktur'")->row_array();
		...
		if($this->form_validation->run()==false){
			...
			$data_ongkir = 	explode("|",$this->input->post("layanan"));
			$ongkir = $data_ongkir[0];
			if($order['kode_kupon'] != '' && $ongkir < $order['discount_ongkir']){
				// Update pemesanan
				$this->db->set('discount_ongkir',$ongkir);
				$this->db->where('nofaktur',$this->session->userdata('nofaktur'));
			...
			$data = [
				...
				'layanan' => $data_ongkir[1]." - ".$data_ongkir[2]
			];
