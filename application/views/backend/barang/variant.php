        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
              <form method="POST" id="form">
                ...
                <div class="form-group grp-warna" style="display: none;">
                  Warna
                  <input type="color" class="form-control" name="warna" id="warna">
                  <small class="text-danger" id="error-warna"></small>
                </div>
            </div>
  ...
  <script>
    ...
    var Controller = (function (uiCtr){
      ...
      var setupEventListener = function(){
        ...
        $(dom.warna).on('change',function(){
          uiCtr.bersihError(dom.errorWarna);
        });
      ...
      function tampilTabel(){
        ...
        table = $("#tabel").dataTable({
          initComplete: function() {
              ...
              processing: true,
              serverSide: true,
              ajax: {"url": "<?php echo base_url().'admin/Variant/getVariant'?>", "type": "POST"},
                columns: [                     
                  {
                    "data": "id",
                    "orderable": false,
      ...
      function hapus(id) {
        $.ajax({
          type:'POST',
          data:{id:id},
          url:"<?=base_url('admin/Variant/deleteVariant')?>",
          dataType:'JSON',
      ...
      function insert(){
        ...
        $.ajax({
          type:'POST',
          data:form_data,
          url:base_url+"admin/Variant/insertVariant",
          processData: false,
          contentType: false,
          dataType:"JSON",
          success:function(data){
            if(data.pesan == "Berhasil"){
              ...
              uiCtr.tampilError(dom.errorWarna,data.errorwarna);
            }
          }
        });
      }
      ...
      function ambilData(id){
        $.ajax({
          type:'POST',
          data:'id='+id,
          url:"<?=base_url('admin/Variant/ambilByIdVariant')?>",
          dataType:'JSON',
          success:function(data){
            $('#id').val(data['id']);
            uiCtr.isiKolom(dom.nama,data['nama']);
            if(data['id_kategori_variant']==2){
              uiCtr.isiKolom(dom.warna,data['kode_warna']);
              $('.grp-warna').show();
            }
            uiCtr.isiKolom(dom.kategori,data['id_kategori_variant']);
          }
        });
      }
